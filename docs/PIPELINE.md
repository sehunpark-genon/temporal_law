...
> 전체 흐름(보고용)은 [OVERVIEW.md](OVERVIEW.md), 본문·하이퍼링크 추출 로직은 [COLLECTION.md](COLLECTION.md) 참고. 이 문서는 "전체 목록을 받아 어떻게 자동으로 돌리고 적재·갱신하나".
...
```
> ⚠️ 법률 전체(1,714건)는 법령당 본문+lsDelegated+정관 렌더(~수십 초)라 **전체 backfill 은 수 시간** 걸린다. 워커를 백그라운드 데몬으로 띄우고 `backfill` 을 반복 호출(이어받기)하거나 `BACKFILL_BATCH`/워커 수를 조절.

---

## 7. ⚠️ 이 PC 현황

이미 다른 `temporal`(7233)·`temporal_ui`(8080)·`collect_app_db`(5433) 가 떠 있어 `docker compose up -d` 전체는 포트 충돌. 그래서:
- **기존 Temporal(7233) 재사용** (고유 task queue `law-pipeline` 라 비파괴적).
- 앱 DB 는 새로 띄운 전용 **`lawdb`(localhost:5544)** 사용 — 기존 `collect_app_db` 는 건드리지 않음.

---

## 8. 검증 완료 (실측 · lawdb 5544)

- `discover` → **law_catalog 에 법률 1,714건** 적재 (+ collect_state pending 1,714).
- `backfill 3` → 2 done / 1 failed (`목내용` 중첩 리스트 버그) → **버그 수정 후 재실행 → 그 법령 attempts=2 로 done**. = 재처리/이어받기 동작 확인.
- 법령별 실패 격리(나머지는 계속), 상태·시도횟수 추적 정상.
