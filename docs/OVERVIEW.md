...
- **링크 취약점**: 조례/정관 URL 은 **제목 기반**(`자치법규/{제목}`, `학칙공단/{제목}`) → 제목이 바뀌면 URL 이 깨질 수 있음. 조례는 `target_mst`(일련번호) 저장돼 있어 MST 기반 보정 여지, **정관은 `target_mst` 가 비어** 더 취약.

**보완 방법(현재 A만 적용):**
| | 방법 | 비용 |
|---|---|---|
| A (적용) | **부모 기준** — 부모 법 MST 변경 시 lsDelegated/렌더 재수집 → 조례/정관 갱신 | 공짜. 단독 변경은 놓침 |
| B (옵션) | **조례·정관 보유 법만 주기(주/월) 강제 재수집** — 부모 MST 안 바뀌어도 | 그 법 수에 비례(일부라 감당 가능) |
| C (비추천) | 조례 일련번호로 개별 현재상태 확인 | 조례 수만큼 호출(무겁고, 정관엔 API 없어 불가) |

→ 권장: **A + (필요 시) B**. 완벽한 독립 추적은 비용 과다라 **"링크백(최신 가리킴) + 주기 재수집"** 으로 실용 타협.

### 6-4. 기타
- **정관은 브라우저(Chrome) 필수** — JS 렌더 페이지에서만 나옴. Selenium/Playwright 도 결국 브라우저 필요. 브라우저 없으면 정관만 누락.
- **인용/자기참조는 텍스트 휴리스틱**(같은 법·괄호·절경계 보정) — 드물게 오귀속 가능. 본문에 트리거 문구만 있고 등록문서 0건인 **빈 위임링크**는 제외(누락 아님).

**미적용 보완안:** 인용/정관 `target_mst` 채우기(인용=catalog 조인, 정관=팝업 seq 추출), 조례/정관 URL 을 MST 기반으로 전환, 위 B(주기 재수집). 필요해지면 추가.
